...
const getTypeEffectiveness = (types, typeChart = null) => {
  let typeRelations = {}

  if (types.secondary) {
    const typeOne = buildTypeWeaknesses(types.primary)
    const typeTwo = buildTypeWeaknesses(types.secondary)

    for (let type in typeOne) {
      typeRelations[type] = typeOne[type] * typeTwo[type]
    }
  } else {
    typeRelations = buildTypeWeaknesses(types.primary);
  }

  if (typeChart) {
    for (let type in typeRelations) {
      typeChart[`${typeRelations[type]}x`].push(type)
    }
